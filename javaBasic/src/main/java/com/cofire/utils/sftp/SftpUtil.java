...
public class SftpUtil {
    ...
    public ChannelSftp connectSFTP(String host, String username, String password, int port, String privateKey) throws JSchException {
        /*
         * Properties prop= BaseSupport.CommonUtil.getProperties("/configs/sftp.properties"); String host =
         * prop.getProperty("host"); String username = prop.getProperty("username"); String password =
         * prop.getProperty("password"); int port = Integer.valueOf(prop.getProperty("port"));
         */
        JSch jsch = new JSch();
        ...
        try {
            if (privateKey != null && !"".equals(privateKey)) {
                /*
                 * //使用密钥验证方式，密钥可以使有口令的密钥，也可以是没有口令的密钥 if (passphrase != null && "".equals(passphrase)) {
                 * jsch.addIdentity(privateKey, passphrase); } else {
                 */
    ...
    /**
     * 获取连接 host;//sftp服务器ip username;//用户名 password;//密码 privateKey;//密钥文件路径 passphrase;//密钥口令 port =
     * 22;//默认的sftp端口号是22
     * 
     * @return channel
    ...
    public ChannelSftp connectSFTP(String host, int port, String username, String password, String privateKey) throws JSchException {
        ...
        try {
            if (privateKey != null && !"".equals(privateKey)) {
                ...
                /*
                 * if (passphrase != null && "".equals(passphrase)) { jsch.addIdentity(privateKey, passphrase); } else {
                 */
                jsch.addIdentity(privateKey);
                // }
            }
